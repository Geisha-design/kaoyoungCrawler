...
-- 新序列——————————————————————————————————————————————————————————
-- crawler_db.crawler_client definition

CREATE TABLE `crawler_client` (
                                  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
                                  `client_id` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci DEFAULT NULL COMMENT '客户端唯一标识',
                                  `username` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci DEFAULT NULL COMMENT '关联用户名',
                                  `current_url` varchar(500) DEFAULT '' COMMENT '当前浏览器网址',
                                  `support_task_types` varchar(200) DEFAULT '' COMMENT '支持的任务类型（逗号分隔）',
                                  `browser` varchar(20) DEFAULT 'Chrome' COMMENT '浏览器类型',
                                  `connect_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '连接时间',
                                  ...
                                  `last_update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '最后更新时间',
                                  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=115 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='爬虫客户端注册表';


-- crawler_db.crawler_result definition
...
CREATE TABLE `crawler_result` (
                                  ...
                                  UNIQUE KEY `uk_result_id` (`result_id`),
                                  KEY `idx_task_id` (`task_id`)
) ENGINE=InnoDB AUTO_INCREMENT=71 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='爬取结果表';


-- crawler_db.crawler_scheduled_task definition
...
CREATE TABLE `crawler_scheduled_task` (
                                          ...
                                          `script_id` varchar(100) NOT NULL COMMENT '关联脚本ID',
                                          `domain` varchar(200) NOT NULL COMMENT '目标域名正则',
                                          `interval` bigint NOT NULL COMMENT '执行间隔（毫秒）',
                                          `enabled` tinyint(1) DEFAULT '1' COMMENT '是否启用（1=启用，0=禁用）',
                                          `execute_on_idle` tinyint(1) DEFAULT '0' COMMENT '是否仅在空闲时执行（0=否，1=是）',
                                          `task_name` varchar(100) DEFAULT '' COMMENT '任务名称',
                                          `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                                          ...
                                          KEY `idx_client_id` (`client_id`),
                                          KEY `idx_script_id` (`script_id`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='定时爬取任务表';


-- crawler_db.crawler_script definition
...
CREATE TABLE `crawler_script` (
                                  ...
                                  KEY `idx_task_type` (`task_type`),
                                  KEY `idx_domain_pattern` (`domain_pattern`)
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='爬取JS脚本表';


-- crawler_db.crawler_script_push_log definition
...
CREATE TABLE `crawler_script_push_log` (
                                           ...
                                           UNIQUE KEY `uk_push_id` (`push_id`),
                                           KEY `idx_client_id` (`client_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='JS脚本下发日志表';


-- crawler_db.crawler_task definition

CREATE TABLE `crawler_task` (
                                `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
                                `task_id` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci DEFAULT NULL COMMENT '任务唯一标识',
                                `client_id` varchar(100) NOT NULL COMMENT '关联客户端ID',
                                `task_type` varchar(50) NOT NULL COMMENT '任务类型（即时/定时）',
                                `script_id` varchar(100) NOT NULL COMMENT '关联脚本ID',
                                `timeout` int DEFAULT '30000' COMMENT '任务超时时间（毫秒）',
                                `params` varchar(500) DEFAULT '' COMMENT '任务额外参数（JSON格式）',
                                `execute_on_idle` tinyint(1) DEFAULT '0' COMMENT '是否仅在空闲时执行（0=否，1=是）',
                                `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                                `status` varchar(20) DEFAULT 'pending' COMMENT '任务状态（pending/processing/success/fail）',
...
-- crawler_db.crawler_user definition
...
CREATE TABLE `crawler_user` (
                                ...
                                PRIMARY KEY (`id`),
                                UNIQUE KEY `uk_username` (`username`)
) ENGINE=InnoDB AUTO_INCREMENT=15 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='爬虫系统用户表';
...
-- 数据初始化
INSERT INTO crawler_client (id, client_id, username, current_url, support_task_types, browser, connect_time, status, idle_status, last_update_time) VALUES(0, '', '', '', '', 'Chrome', CURRENT_TIMESTAMP, 'online', 0, CURRENT_TIMESTAMP);
INSERT INTO crawler_result (id, result_id, task_id, client_id, crawl_data, crawl_status, crawl_time, storage_time) VALUES(0, '', '', '', '', 'success', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
INSERT INTO crawler_scheduled_task (id, task_key, client_id, username, script_id, `domain`, `interval`, enabled, execute_on_idle, task_name, create_time, update_time) VALUES(0, '', '', '', '', '', 0, 1, 0, '', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
INSERT INTO crawler_script (id, script_id, task_type, script_content, description, domain_pattern, update_time) VALUES(0, '', '', '', '', '.*', CURRENT_TIMESTAMP);
INSERT INTO crawler_script_push_log (id, push_id, client_id, script_ids, push_type, push_status, push_time, remark) VALUES(0, '', '', '', '', '', CURRENT_TIMESTAMP, '');
INSERT INTO crawler_task (id, task_id, client_id, task_type, script_id, timeout, params, execute_on_idle, create_time, status, priority) VALUES(0, '', '', '', '', 30000, '', 0, CURRENT_TIMESTAMP, 'pending', 1);
INSERT INTO crawler_user (id, username, password, create_time, update_time) VALUES(0, '', '', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);